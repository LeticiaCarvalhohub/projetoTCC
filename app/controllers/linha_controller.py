...
from app.models.linha import insert_linhas, update_linha, delete_linhas

linha_bp = Blueprint("linha", __name__)

@linha_bp.route("/linhas", methods=["POST"])
def adicionar_linha():
...
@linha_bp.route("/linhas/<int:codigo_linha>", methods=["PUT"])
...
@linha_bp.route("/linhas/<int:codigo_linha>", methods=["DELETE"])
